...
| `storage`                       | Save the user's own data locally: their saved Salesforce orgs (org name, username, password, SSO/My Domain URL, optional 2FA key) and preferences (theme, tab colors). Stored only on the user's device, never transmitted to us or any third party, and can be encrypted with a master passphrase. Required so saved logins/settings persist between sessions.                                                            |
